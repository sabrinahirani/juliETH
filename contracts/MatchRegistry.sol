...
contract MatchRegistry {
    ...
    mapping(address => address[]) private matches;

    event MatchVerified(address indexed potentialMatch, address indexed sender, bool result);
    ...
    function verifyMatch(address potentialMatch, uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC) public returns (bool) {
        ...
        (
            ...
            uint8 hobby,
            bool exists
        ) = preferencesRegistry.getPreferences(potentialMatch);
        require(exists, "No preferences");

        // construct public input
        uint256[9] memory input;
        input[0] = registeredCommitment;
        input[1] = minAge;
        input[2] = maxAge;
        ...
        emit MatchVerified(potentialMatch, msg.sender, matched);
        return matched;
    }
}